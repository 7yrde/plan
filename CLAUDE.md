...
- **접속 경로**: http://localhost/dashboard/
- **Jira 프로젝트**: PLAN (`https://7yr.atlassian.net/jira/software/projects/PLAN/board`)

## Docker 배포

- `output: "standalone"` + `basePath: "/dashboard"` 설정 필수
- Alpine 이미지에서 better-sqlite3 빌드를 위해 빌더 스테이지에 `apk add python3 make g++`
- 포트 3000, nginx가 `/dashboard/` 라우트로 프록시

## 7YR 슈퍼프로젝트 내 위치

- 서브모듈 경로: `./7-dashboard`
- 원래 `qkrwogk/7-dashboard` → `7yrde/7-dashboard`로 이전
